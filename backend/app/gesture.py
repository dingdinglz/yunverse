...
class GestureStore:
    ...
    def resolve_technique(self) -> tuple[dict, list[str]]:
        """解析当前技法。
        ...
        technique_code = self._mapping.get(state.gestureCode or "")
        if not technique_code or not self._registry.has(technique_code):
            # 映射缺失（手势→技法文档待补充）：安全回退到 normal
            return self._registry.object(self._fallback), warnings

        return self._registry.object(technique_code), warnings

    def resolve_technique_for(self, gesture_code: str | None) -> dict:
        """给定手势编码直接解析技法对象（用于上报接口回显）。"""
        code = self._mapping.get(gesture_code or "")
